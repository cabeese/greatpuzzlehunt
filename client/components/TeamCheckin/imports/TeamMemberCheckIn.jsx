...
class TeamMemeberCheckIn extends Component {
    ...
    return (
      <Grid.Row columns={2} key={userId}>
        <Grid.Column>
          ...
          <Button disabled={!paid || checkInConfirmed} basic={checkedIn}
                  color={checkedIn ? "grey" : "green"}
                  content={checkedIn ? "Cancel" : "Here!"}
                  onClick={async () => await this._toggleCheckin(userId)} />
        </Grid.Column>
        <Grid.Column>
  ...
  async _toggleCheckin(userId) {
    try {
      await Meteor.callAsync('team.checkin.user', userId);
    } catch (error) {
      alert(error.reason);
    }
  }
