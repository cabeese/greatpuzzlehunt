...
async function buildUserReport() {
  const USER_FIELDS = ["createdAt", "firstname", "lastname", "email",
		       "gameMode", "playingPuzzleHunt",
		       "playingTreasureHunt", "accountType", "paid",
		       "ticketUsed", "lookingForTeam", "teamName",
		       "teamDivision", "age", "city", "state", "address",
		       "zip", "country", "phone", "ecName",
		       "ecRelationship", "ecPhone", "ecEmail",
  ...
  let playersGPHInPerson = 0;
  let playersGPHVirtual = 0;
  let playersTreasureHunt = 0;
  let playersBoth = 0;
  let playersTotal = 0;
  let usersVolunteer = 0;
  let gameDayInPerson = 0;
  ...
  all_users.forEach(item => {
    ...
    if (item.accountType == 'VOLUNTEER') {
      ...
      playersTotal += 1;
      if (item.playingPuzzleHunt && item.playingTreasureHunt) {
	playersBoth += 1;
      }
      if (item.gameMode == 'INPERSON') {
	peopleInPerson += 1;
	if (item.playingPuzzleHunt) {
          playersGPHInPerson += 1;
	}
	if (item.playingTreasureHunt) {
          playersTreasureHunt += 1;
	}
      } else {
	if (item.playingPuzzleHunt) {
          playersGPHVirtual += 1;
  ...
  csv += "GPH players in person," + playersGPHInPerson.toString() + "\n";
  csv += "GPH players virtual," + playersGPHVirtual.toString() + "\n";
  csv += "GPH Players total," + (playersGPHVirtual + playersGPHInPerson).toString() + "\n";
  csv += "Treasure hunt players," + playersTreasureHunt.toString() + "\n";
  csv += "Players in both," + playersBoth.toString() + "\n";
  csv += "Players total," + playersTotal.toString() + "\n";
  csv += "Volunteers," + usersVolunteer.toString() + "\n";
  csv += "Users total," + (playersTotal + usersVolunteer).toString() + "\n";
  csv += "Game day people in person," + (playersGPHInPerson + usersVolunteer).toString() + "\n";
