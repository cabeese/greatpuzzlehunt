...
export function convertCashnet(reqBody) {
    ...
    // International shipping is WEIRD and it appears that for each shipped item
    // (eg shirts but not ticket codes), an international shipping line item is
    // added in the CashNet data. Thus an order for 5 hoodies will include 5
    // "international shipping" line items. Unfortunately, I think this means that
    // it's not possible to directly correlate a specific shipping request to each
    // specific item. Instead, we assume that if there is a request for
    // international shipping, then the WHOLE order must be shipped internationally
    // so we go through and set the flag appropriately for each gear item.
    // (Each gear item has this separately so that we can easily include it on the
    // gear reports used to handle gear purchasing + logistics on our end.)
    if (internationalShipping) {
        items.gear.forEach(item => item.internationalShipping = true);
    }
