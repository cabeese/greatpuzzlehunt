...
import { requireUser, requireVolunteer, NonEmptyString, requirePattern, notAfterCheckIn, checkMsg, requireAdmin } from '../imports/method-helpers.js';
...
Meteor.methods({
  ...
  'teams.upsert'(team) {
    ...
    // If setting inPerson=true, make sure all members are allowed to play in-person
    if (team.inPerson) {
      const members = team.members || Teams.findOne({_id: team._id}).members;
      members.forEach(member_id => {
        const user = Meteor.users.findOne({ _id: member_id});
          if (!user.inPersonAllowed) {
            throw new Meteor.Error(400, "Can't set 'in-person' mode: at least one team member is registered to play virtually");
          }
      });
    }
  ...
  'teams.join'(teamId, password) {
    ...
    else if (team.inPerson && !user.inPersonAllowed) {
      throw new Meteor.Error(400, 'You must have an in-person ticket code to join this team');
    }
  ...
  'team.puzzle.answer'(puzzleId, answer) {
    ...
    let teamNameShort = team.name;
    if (teamNameShort.length > 23) {
      teamNameShort = `${teamNameShort.substr(0,20)}...`
    }
    ...
    if (cleanAnswer === masterPuzzle.answer.replace(/ /g, "").toLowerCase()) {
      ...
      Meteor.logger.info(`Team "${teamNameShort}" completed puzzle "${masterPuzzle.name}" at ${endTime}`);
  ...
  'team.puzzle.takeHint'(puzzleId, hintIndex) {
    ...
    Meteor.logger.info(`Team _id=${user.teamId} is taking hint ${hintIndex}`);
  ...
  'team.startPuzzleVirtuallyByPlayer'(teamId, puzzleId) {
    ...
    check(puzzleId, String);
    if (!Meteor.isServer) return true;
    Meteor.logger.debug(`Team _id=${teamId} attempting to start puzzle _id=${puzzleId}`);
    ...
    Meteor.logger.info(`Team "${teamNameShort}" is starting puzzle "${puzzle.name}"`);
