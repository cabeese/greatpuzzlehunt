...
import { getPuzzleScore, getPuzzleScoreAndTimeout } from '../imports/puzzle-helpers';
import { getShortName } from '../imports/util';

import { Invites } from './invites.js';
import { Gamestate } from './gamestate-collection.js';
import { Puzzles } from './puzzles.js';
import { THCheckpoints } from './thcheckpoints.js';
...
Meteor.methods({
  ...
  async 'teams.upsert'(team) {
    ...
    const { name, password, division, inPerson, playingPuzzleHunt, playingTreasureHunt, lookingForMembers } = team;
    ...
    requirePattern(playingPuzzleHunt, Boolean, "Playing puzzle hunt must be true or false");
    requirePattern(playingTreasureHunt, Boolean, "Playing treasure hunt must be true or false");
  ...
  async 'team.begin'() {
    ...
    const startingPuzzle = sample(puzzles.filter(p => p.stage === 0))
    if (!startingPuzzle) {
      Meteor.logger.error("Couldn't find any puzzles with stage 0!");
      throw new Meteor.Error(404, "Configuration error found when setting your first puzzle. We will address this shortly.");
    }
    const startLocation = startingPuzzle.name;
  ...
  async 'team.startTreasureHunt'(teamId) {
    check(teamId, String);
    if (!Meteor.isServer) return true;

    const team = await Teams.findOneAsync(teamId);
    if (!team) {
      throw new Meteor.Error(400, `No team found with id ${teamId}`);
    }
    Meteor.logger.info(`Team '${teamId}' starting treasure hunt play`);
    return await Teams.updateAsync(teamId, {
      $set: {
	startedTreasureHunt: true,
	completedCheckpoint: null
      }
    });
  },

  async 'team.clearTreasureHunt'(teamId) {
    check(teamId, String);
    if (!Meteor.isServer) return true;
    ...
    Meteor.logger.info(`Team '${teamId}' clearing treasure hunt play`);
    return await Teams.updateAsync(teamId, {
      $set: {
	startedTreasureHunt: false,
	completedCheckpoint: null
  ...
  async 'team.treasure.answer'(checkpointId, answer) {
    ...
    const team = await Teams.findOneAsync(user.teamId);
    if (!team) {
      throw new Meteor.Error(400, `Oops, no team found with id ${user.teamId}`);
    }
    Meteor.logger.info('Got team:');
    Meteor.logger.info(team);
    const teamNameShort = getShortName(team.name);
    ...
    // get the correct codeword for this checkpoint
    const cknumid = parseInt(checkpointId, 10);
    const checkpoint = await THCheckpoints.findOneAsync({ sequence: cknumid});
    Meteor.logger.info('got checkpoint:');
    Meteor.logger.info(checkpoint);
    if (!checkpoint) {
      throw new Meteor.Error(400, `Oops, no checkpoint found with id ${checkpointId}`);
    }

    const nonCharacterDigit = /[^a-zA-Z0-9]/ug
    const cleanAnswer = answer.replace(nonCharacterDigit, '').toLowerCase();
    Meteor.logger.info(`got clean answer: ${cleanAnswer}`);

    const expected = checkpoint.codeword.replace(nonCharacterDigit, '').toLowerCase();
    Meteor.logger.info(`got expected answer: ${expected}`);

    if (!checkpoint.hasCodeword || (cleanAnswer === expected)) {
      Meteor.logger.info(`Team "${teamNameShort}" completed treasure hunt checkpoint "${checkpointId}"`);

      // update the team info
      await Teams.updateAsync(user.teamId, {
        $set: {
	  completedCheckpoint: cknumid
	},
      });
      return { success: true, message: 'Correct!' };
    } else {
      Meteor.logger.info('answer did not match');
      return { success: false, message: 'Nope, keep trying!' };
    }
  },
  ...
  async 'admin.team.togglePlayingPuzzleHunt'(teamId) {
    check(teamId, String);
    requireAdmin();
    if (!Meteor.isServer) return true;
    ...
    const team = await Teams.findOneAsync(teamId);
    if (!team) throw new Meteor.Error(400, `Oops, no team found with id ${teamId}`);
    const currVal = team.playingPuzzleHunt || false;
    Meteor.logger.info(`Admin toggling playingPuzzleHunt value for team ${teamId}`);

    return await Teams.updateAsync(teamId, {
      $set: {
        playingPuzzleHunt: !currVal,
      }
    });
  },

  async 'admin.team.togglePlayingTreasureHunt'(teamId) {
    check(teamId, String);
    requireAdmin();
    if (!Meteor.isServer) return true;
    ...
    const team = await Teams.findOneAsync(teamId);
    if (!team) throw new Meteor.Error(400, `Oops, no team found with id ${teamId}`);
    const currVal = team.playingTreasureHunt || false;
    Meteor.logger.info(`Admin toggling playingTreasureHunt value for team ${teamId}`);

    return await Teams.updateAsync(teamId, {
      $set: {
        playingTreasureHunt: !currVal,
      }
    });
  },
  ...
  async 'admin.team.updatePuzzlePlay'(teamId, puzzle) {
    ...
    const puzzleName = team.puzzles[i].name;
