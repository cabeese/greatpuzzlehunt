...
Meteor.methods({
  ...
  'teams.upsert'(team) {
    ...
    const { name, password, division, members, lookingForMembers, bio } = team;
    ...
    if(members && members.length > 5) {
      requirePattern(lookingForMembers, false, "You cannot look for members if your team is full.");
    }
    if(lookingForMembers) {
      requirePattern(bio, NonEmptyString, "If you are looking for members then you must have a bio!");
    }
    ...
    // if team already exists check the update is coming from a current memeber:
    if (!isNewTeam) {
  ...
  'teams.join'(teamId, password) {
    ...
    if(team.members.length == 5) { // Disable lookForMembers if the user will make it so the team has 6 (or more) members.
      Teams.update(teamId, { $push: { members: user._id }, $set: { lookingForMembers: false, updatedAt: new Date() } });
    } else {
