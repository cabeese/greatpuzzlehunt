...
Meteor.methods({
  'team.owner'(userId) {
    ...
    const owner = Meteor.users.findOneAsync(userId);
    return owner ? {
      name: owner.name,
  ...
  'teams.upsert'(team) {
    ...
    const duplicateNameTeam = Teams.findOneAsync(opts);
    ...
    if (!isNewTeam) {
      const thisTeam = Teams.findOneAsync({_id: team._id });
  ...
  'teams.removeMember'(member) {
    ...
    const user = Meteor.users.findOneAsync({ _id: this.userId });
    const memberToDelete = Meteor.users.findOneAsync({ _id: member._id });
    const team = Teams.findOneAsync({ _id: user.teamId });
  ...
  'teams.join'(teamId, password) {
    ...
    const team = Teams.findOneAsync(teamId);
    if (!team) {
  ...
  'teams.delete'() {
    ...
    const team = Teams.findOneAsync(user.teamId);
    if (user._id !== team.owner) {
  ...
  'team.checkin.user'(userId) {
    ...
    const userToToggle = Meteor.users.findOneAsync(userId);
  ...
  'team.puzzle.giveUp'(puzzleId, teamId){
    ...
    const masterPuzzle = Puzzles.findOneAsync(puzzleId);
  ...
  'team.startPuzzleVirtuallyByPlayer'(teamId, puzzleId) {
    ...
    const i = findIndex(team.puzzles, (p) => p.puzzleId === puzzleId);
    const puzzle = team.puzzles[i];
    const masterPuzzle = Puzzles.findOneAsync(puzzle.puzzleId);
