...
Meteor.methods({
  ...
  'team.checkin.virtuallyByPlayer'(teamId) {
    ...
    const teamOwner = Meteor.users.findOne({_id: team.owner});
    if (!teamOwner) { throw new Meteor.Error(400, "Team owner does not exist!"); }
    if (!teamOwner.checkedIn) {
      throw new Meteor.Error(400, "Your team leader must be checked in in order to play");
    }
