...
Meteor.methods({
  ...
  async 'admin.team.updatePuzzlePlay'(teamId, puzzle) {
    ...
    const ps = JSON.stringify(puzzle);
    Meteor.logger.info(`update puzzle play, team ${teamId} puzzle ${ps}`);

    const team = await Teams.findOneAsync(teamId);
    if (!team) {
      throw new Meteor.Error(400, `Oops, no team found with id ${teamId}`);
    }
    const ts = JSON.stringify(team);
    Meteor.logger.info(`got team: ${ts}`);
    const tps = JSON.stringify(team.puzzles);
    Meteor.logger.info(`got team puzzles: ${tps}`);
    const { puzzleId } = puzzle;
    Meteor.logger.info(`got puzzle id: ${puzzleId}`);
    const i = findIndex(team.puzzles, (p) => p.puzzleId === puzzleId);
    if (i < 0) {
