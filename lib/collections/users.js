...
// If this is a US or Canada addres, check that the phone numbers
// conform to the NANP ten-digit standard; otherwise, allow any phone
// number value (including empty)
// Note that this does allow for invalid phone numbers as it does not
// check the detailed syntax of NANP numbers, but it should catch
// egregious errors
function checkPhoneNumbers(country, phone, ecphone) {
    if (isUnitedStates(country) || isCanada(country)) {
        check(phone, String)
        check(ecphone, String)
        const phonePunctuation = /()-\d/ig;
        if (!phone.match(phonePunctuation)) {
            throw new Meteor.Error(400, 'Invalid phone number');
        }
        if (!ecphone.match(phonePunctuation)) {
            throw new Meteor.Error(400, 'Invalid emergency contact phone number');
        }
        const nonDigits = /\D/g;
        if (phone.replaceAll(nonDigits, '').length != 10) {
            throw new Meteor.Error(400, 'Phone number must be ten digits');
        }
        if (ecphone.replaceAll(nonDigits, '').length != 10) {
            throw new Meteor.Error(400, 'Emergency contact phone number must be ten digits');
        }
    }
}
...
Meteor.methods({
    ...
    // check that phone numbers make sense
