...
export const errorMap = {
  ...
  lftBioMissing: 'You must have a bio if you have "looking for a team?" enabled',
};
...
function checkUserProfileData(data) {
  try {
    ...
    // If looking for team is enabled then there must be a bio
    if (data.lookingForTeam == true) {
      if (!Match.test(data.bio, NonEmptyString)) {
        // because of the error checking a custom Match error must be created
        // this is so a custom error message can be displayed to the website.
        // The error message is named 'lftBioMissing' in the errorMap constant
        let error = new Error('**custom** Match error: Failed Match.Where validation in field bio');
        error.path = 'lftBioMissing';
        error.sanitizedError = new Meteor.Error(400, 'Match failed');
        throw error
      }
    }
  } catch(ex) {
