...
// Ensure at least one activity is selected, and that "Treasure Hunt" participants are in-person.
function checkActivities(playingPuzzleHunt, playingTreasureHunt, gameMode, gamestate) {
  ...
  if (playingTreasureHunt && gameMode !== "INPERSON") {
    throw new Meteor.Error(400, "The Treasure Hunt is only open to in-person participants");
  }

  // Make sure registration allows for this action
  if (playingTreasureHunt && !gamestate.registrationTreasureHuntOpen) {
    throw new Meteor.Error(400, "Sorry, Treasure Hunt registration is closed at this time.");
  }
  if (playingPuzzleHunt && !gamestate.registrationInPersonOpen &&
...
Meteor.methods({
  async 'user.register'(data) {
    ...
    if (!isVolunteer && data.gameMode === "INPERSON") {
      if (gamestate.registrationInPersonOpen === false) {
        throw new Meteor.Error(400,
          "Sorry, We can no longer accept in-person players at this time");
      }
    }

    // Check that at least one "activity" is selected and match gameMode
    if (!isVolunteer) {
