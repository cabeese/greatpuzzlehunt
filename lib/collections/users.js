...
const MATCHMAKING_MESSAGES_LIMIT = 30;
...
Meteor.methods({
  ...
  'user.matchmaking-message'(email, message) {
    ...
    const currentUser = Meteor.userId();
    const user = Meteor.users.findOne(currentUser);
    if (user.matchmakingMessagesSent >= MATCHMAKING_MESSAGES_LIMIT) {
      throw new Meteor.Error(
        403,
        "You've sent too many messages to other players. Please contact Support"
      );
    }
    emailUserMessage(user, email, message);
    return Meteor.users.update(user._id, { $inc: { matchmakingMessagesSent: 1 }});
  },
