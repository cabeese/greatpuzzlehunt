...
Meteor.methods({
  'user.register'(data) {
    ...
    if (!isVolunteer && data.gameMode === "INPERSON") {
      if (Gamestate.findOneAsync({}).registrationInPersonOpen === false) {
        throw new Meteor.Error(400,
  ...
  'user.matchmaking-message'(email, message) {
    ...
    const currentUser = Meteor.userId();
    const user = Meteor.users.findOneAsync(currentUser);
  ...
  'user.redeem'(ticketCode) {
    ...
    const user = Meteor.users.findOneAsync(currentUserId);
    // validate user needs to redeem
    if (user.paid) { throw new Meteor.Error(400, 'Your account is already active!'); }

    const ticket = Tickets.findOneAsync({ code: ticketCode });
    // Validate ticket
