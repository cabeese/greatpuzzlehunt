...
export const errorMap = {
  ...
  holdHarmless: 'You must accept the Acknowledgement of Risk & Hold Harmless Agreement.',
  bio: 'Your bio must be a string of text.',
  lookingForTeam: '"Looking for team" is a Boolean option (true/false).',
  playingTreasureHunt: 'Playing is a Boolean option (true/false).',
  playingPuzzleHunt: 'Playing is a Boolean option (true/false).',
};
...
// Validate the ever-growing matrix of registration options.
//
// * Check for violations based on which registration options are open
// * Check that at least one "activity" is selected and matches gameMode
// * etc.
function checkActivities(playingPuzzleHunt, playingTreasureHunt, gameMode, gamestate) {
  // Dummy check: make sure at least one activity is selected
  if (!playingPuzzleHunt && !playingTreasureHunt) {
    throw new Meteor.Error(400, "You must select at least one Activity (Puzzle Hunt or Treasure Hunt)");
  }

  // Validation for anyone doing the Puzzle Hunt
  //
  // Much of this should already be validated on the client side, but check here to be safe.
  if (playingPuzzleHunt) {
    if (!gamestate.registrationInPersonOpen &&
        !gamestate.registrationVirtualOpen) {
      throw new Meteor.Error(400, "Sorry, Puzzle Hunt registration is closed at this time.");
    }

    if (gameMode === "INPERSON" && gamestate.registrationInPersonOpen === false) {
      throw new Meteor.Error(400, "Sorry, We can no longer accept in-person players at this time");
    }
  }

  // Validation for anyone doing the Treasure Hunt
  if (playingTreasureHunt) {
    if (gameMode !== "INPERSON") {
      throw new Meteor.Error(400, "The Treasure Hunt is only open to in-person participants");
    }

    if (!gamestate.registrationTreasureHuntOpen) {
      throw new Meteor.Error(400, "Sorry, Treasure Hunt registration is closed at this time.");
    }
  }
}
...
Meteor.methods({
  async 'user.register'(data) {
    ...
    // 3. Check game selections against gamestate and more
    const gamestate = await Gamestate.findOneAsync({});
    if (!isVolunteer) {
      checkActivities(data.playingPuzzleHunt, data.playingTreasureHunt,
                      data.gameMode, gamestate);
    }
  ...
  async 'user.update.account'(fields) {
    ...
    const { firstname, lastname, gameMode, lookingForTeam, bio,
    ...
    const gamestate = await Gamestate.findOneAsync({});
    if (!isVolunteer) {
      checkActivities(playingPuzzleHunt, playingTreasureHunt,
                      gameMode, gamestate);
    }
