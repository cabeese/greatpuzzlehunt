...
export const PromoCodes = new Mongo.Collection('promo_codes');
...
Meteor.methods({
  ...
  async 'promo_codes.create'() {
    requireAdmin();

    if (!Meteor.isServer) return true;

    return await PromoCodes.insertAsync({
      name: 'New Promo Code',
      code: '',
      units: 6,
      used: 0,
  ...
  async 'promo_codes.update'(id, data) {
    try {
      check(id, String);
      check(data, {
        name: String,
    ...
    return await PromoCodes.updateAsync(id, { $set: {
      name,
  ...
  async 'promo_codes.delete'(id) {
    ...
    return await PromoCodes.removeAsync(id);
  },
