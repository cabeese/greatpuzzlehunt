  ...
  if (Meteor.isServer) {
    ...
    if (!gameState) {
      await Gamestate.insertAsync({
        gameplay: false,
	treasureplay: false,
        registrationInPersonOpen: false,
        registrationVirtualOpen: false,
        registrationTreasureHuntOpen: false,
        leaderboard: false,
	givingURL: "https://foundation.wwu.edu/greatpuzzlehunt",
      });
    } else {
...
Meteor.methods({
  async 'admin.gamestate.toggleField'(field) {
    ...
    const currentGamestate = await Gamestate.findOneAsync({});
    const oldState = currentGamestate[field];
    if (oldState === undefined && Meteor.isServer) {
      Meteor.logger.warn(`Gamestate field ${field} is undefined, but is being set!`);
    }
    await Gamestate.updateAsync({ _id: currentGamestate._id }, {
      $set: {
        [field]: !oldState,
      }
    });
    if (Meteor.isServer) {
      Meteor.logger.info(`${admin.name} set "${field}" to ${!oldState}`);
    }
  },
  ...
  async 'admin.gamestate.setGivingURL'(url) {
    requireAdmin();
    if (!Meteor.isServer) return true;
    check(url, String);
    ...
    Meteor.logger.info(`${user.name} updated the giving URL to {url}`);
  },

  async 'admin.gamestate.setTreasureInfo'(mapurl, logurl) {
    requireAdmin();
    if (!Meteor.isServer) return true;
    check(mapurl, String);
    check(logurl, String);
    ...
    await Gamestate.updateAsync({ _id: currentState._id }, {
      $set: {
        treasureMapURL: mapurl,
	treasureLogURL: logurl
      }
    });

    Meteor.logger.info(`${user.name} updated the treasure hunt map URL to ${mapurl}`);
    Meteor.logger.info(`${user.name} updated the treasure hunt log URL to ${logurl}`);
  },
