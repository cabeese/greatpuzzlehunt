...
const Gamestate = new Mongo.Collection("gamestate");
  ...
  if (Meteor.isServer) {
    ...
    if (!gameState) {
      ...
      if (!gameState.sendReportsTo) {
        Gamestate.updateAsync({_id: gameState._id}, { $set: { sendReportsTo: ["greatpuzzlehunt@gmail.com"]}});
      }
    }
  }
});
...
Meteor.methods({
  async 'admin.gamestate.toggleField'(field) {
    ...
    Gamestate.updateAsync({ _id: currentState._id }, {
      $set: {
  ...
  async 'admin.gamestate.setBannerMarkdown'(markdown) {
    ...
    bannerHtmlUnsanitized = marked.parse(markdown);
